...
function buildResearchContract(mode: ExecutionMode): string {
  const L: string[] = [
    ...
    "VERIFICATION GATE (before you publish): run integrity_report and read the composite grade (0-100 — the mean of the measured dimensions: reproduction matches, claims with evidence, claims with a falsification attempt, originality). If the grade is weak — unsupported claims, failed reproductions, low originality — do NOT publish yet: fix the gap (ground each claim in a cited source with a quote, re-run the failing reproduction, paraphrase copied spans) and re-check. Only publish a run you would defend.",
