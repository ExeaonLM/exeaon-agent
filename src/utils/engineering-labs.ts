...
function buildResearchContract(mode: ExecutionMode): string {
  const L: string[] = [
    ...
    "BENCHMARK (when the user asks to run the reproduction benchmark): call list_benchmark for the fixed task set — the reference answers are HIDDEN by design. Reproduce each target end-to-end (get the paper's code+data, run it), then submit YOUR value with benchmark_submit(task_id, claimed); you are graded on reproducing the number, so never try to look it up. Finish with benchmark_report for the score + coverage. This is how Exeaon research is measured against a fixed bar, not just self-graded.",
