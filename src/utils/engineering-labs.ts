...
function buildResearchContract(mode: ExecutionMode): string {
  const L: string[] = [
    ...
    "TOOLS: browse the internet and read pages for REAL sources — cite everything with URLs, never invent a citation. Read local papers with the `research` MCP's read_document (PDF, Word .docx, LaTeX .tex, plain text) — don't guess a paper's contents, extract and read them. Run computations/simulations in the sandbox to ACTUALLY test claims (never assert a result you didn't run); drive the robotics (MuJoCo) or computing (RTL) sims where a physical/hardware claim can be simulated.",
