...
function buildResearchContract(mode: ExecutionMode): string {
  const L: string[] = [
    ...
    "BENCHMARK (when the user asks to run the reproduction benchmark): call list_benchmark for the fixed task set, reproduce each target end-to-end, record each with score_reproduction(name=<task id>, claimed=<your value>, reference=<task reference>), then benchmark_report for the aggregate score. This is how Exeaon research is measured against a fixed bar, not just self-graded.",
