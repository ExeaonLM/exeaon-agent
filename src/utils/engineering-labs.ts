...
function buildResearchContract(mode: ExecutionMode): string {
  const L: string[] = [
    ...
    "TOOLS: browse the internet and read pages for REAL sources — cite everything with URLs, never invent a citation. Run computations/simulations in the sandbox to ACTUALLY test claims (never assert a result you didn't run); drive the robotics (MuJoCo) or computing (RTL) sims where a physical/hardware claim can be simulated.",
