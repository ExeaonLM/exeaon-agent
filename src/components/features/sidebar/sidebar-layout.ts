...
export const SIDEBAR_ROW_INTERACTIVE_CLASS = {
  active: "bg-tertiary text-[var(--cool-grey-50)] font-normal",
  idle: "text-[var(--oh-muted)] hover:text-[var(--cool-grey-50)] hover:bg-[var(--oh-surface-raised)]",
} as const;
...
export function sidebarCollapsedIconGlyphClassName(active: boolean): string {
  return cn(
    ...
    active
      ? "text-[var(--cool-grey-50)] font-normal"
      : "text-[var(--oh-muted)] group-hover:text-[var(--cool-grey-50)]",
  );
}
